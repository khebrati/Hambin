...
3. Open party manager and choose to create a party or join by party ID. Supported by REQ-008, REQ-009, REQ-010.
4. After creating or joining, move to the party screen. Supported by REQ-011.
5. If the user is the room owner, paste or type a direct video URL with emphasis on easy pasting, then start playback. Supported by REQ-012, REQ-015.
6. If the user is not the owner, view, copy, or inspect the current URL without edit/start permission. Supported by REQ-032.
7. Once playback is active, use custom Material 3 Expressive playback controls for play/pause, forward/backward, and volume. Supported by REQ-017, REQ-026, REQ-027, REQ-028, REQ-039.
8. Use voice chat; when users speak, their avatar shows a speaking indicator. Supported by REQ-023, REQ-024, REQ-025.
9. Press Sync when desired to fast forward local playback to the leading participant. Supported by REQ-020, REQ-021, REQ-022, REQ-038.
10. Leave the party through a visible leave affordance and confirmation prompt. Supported by REQ-034, REQ-037.
