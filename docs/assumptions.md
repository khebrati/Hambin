...
| ASM-014 | Direct video URL validation is represented visually in `webUI` but not implemented there as real media compatibility logic. | The browser client remains the interactive prototype and the stakeholder confirmed direct video URL as the design target. | If real playback validation is required in the web client, technical scope and dependencies would increase. |
